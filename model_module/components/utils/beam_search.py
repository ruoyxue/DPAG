...
from model_module.components.utils.e2e_asr_common import end_detect
from model_module.components.utils.scorer_interface import PartialScorerInterface, ScorerInterface
...
class BeamSearch(torch.nn.Module):
    ...
    def search(
        ...
        if isinstance(x, Tuple) or isinstance(x, List):
            part_ids = torch.arange(self.n_vocab, device=x[0].device)  # no pre-beam
        else:
        ...
        for hyp in running_hyps:
            # scoring
            if isinstance(x, Tuple) or isinstance(x, List):
                weighted_scores = torch.zeros(self.n_vocab, dtype=x.dtype, device=x[0].device)  # no pre-beam
            else:
    ...
    def forward(
        ...
        if maxlenratio == 0:
            ...
            if isinstance(x, Tuple) or isinstance(x, List):
                ...
                maxlen = x.shape[0]
            maxlen = max(1, int(maxlenratio * maxlen))
