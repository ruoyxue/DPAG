...
from .efficient_conformer.modules import InterCTCResModule
...
class Encoder_InterCTC(torch.nn.Module):
    ...
    def __init__(
        ...
        for block_id in range(num_blocks):
            ...
            if interctc_blocks is not None and block_id in interctc_blocks:
                self.encoder.append(InterCTCResModule(
                    dim_model=attention_dim, 
                    vocab_size=odim
                ))
    ...
    def forward(self, xs, masks):
        ...
        for i, block in enumerate(self.encoder):
            if isinstance(block, EncoderLayer):
                xs, masks = block(xs, masks)
            elif isinstance(block, InterCTCResModule):
                xs, logits = block(xs)
                key = "ctc_" + str(i)
                interctc_outputs[key] = logits
            else:
        ...
        return xs, masks, interctc_outputs
