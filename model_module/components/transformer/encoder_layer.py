...
class EncoderLayer(nn.Module):
    ...
    def forward(self, x_input, mask, vis=False, cache=None, return_attention_mask=False):
        ...
        if vis:
            if pos_emb is not None:
                 _, att_scores = self.self_attn(x_q, x, x, pos_emb, mask, rtn_attn=True)
            else:
                 _, att_scores = self.self_attn(x_q, x, x, mask, rtn_attn=True)
            return {
                "encoder": att_scores
            }
        ...
        if return_attention_mask:
            if pos_emb is not None:
                return (x, pos_emb), mask, att_scores
            else:
                return x, mask, att_scores
        else:
            if pos_emb is not None:
