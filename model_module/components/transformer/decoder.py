...
class Decoder(BatchScorerInterface, torch.nn.Module):
    ...
    def forward(self, tgt, tgt_mask, memory, memory_mask, vis=False):
        """Forward decoder.
        ...
        x = self.embed(tgt)
        if vis:
            for layer_id in range(len(self.decoders)):
                if layer_id != len(self.decoders) - 1:
                    x, tgt_mask, memory, memory_mask = self.decoders[layer_id](x, tgt_mask, memory, memory_mask)
                else:
                    vis_result = self.decoders[layer_id](x, tgt_mask, memory, memory_mask, vis=True)
