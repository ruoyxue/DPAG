...
class VideoProcess:
    ...
    def __call__(self, video, landmarks):
        ...
        if (
            preprocessed_landmarks is None
        ...
        for s in sequence:
            assert s is not None, "crop an empty patch."
        return sequence
    ...
    def crop_patch(self, video, landmarks):
        ...
        for frame_idx, frame in enumerate(video):
            ...
            if self.region == 'lip':
                final_landmarks = transformed_landmarks[self.start_idx : self.stop_idx]
            elif self.region == 'face':
                final_landmarks = transformed_landmarks
            else: 
            ...
            if self.resize[0] != self.crop_width or self.resize[1] != self.crop_height:
    ...
    def interpolate_landmarks(self, landmarks):
        ...
        for lm in landmarks:
            assert lm is not None, "not every frame has landmark"
